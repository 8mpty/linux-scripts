...
task_dekstop_rec="xdg-utils fonts-symbola avahi-daemon libnss-mdns anacron eject iw sudo"
...
utils_list="network-manager-gnome blueman"

start_install(){
    echo
    echo -e "${GREEN}${BOLD}Installing Custom XFCE...${RESET}"
    
    apt install $task_desktop $task_dekstop_rec $base_list -y
